    ...
    public function edit($data = [], &$error = '', $is_sql = false)
    {
        ...
        if ($id) {
            ...
            if($admin_info['role_id'] ==1){
                $data['code'] = get_db_config_data('1',"system_city");
            }else{
                $data['code'] = $admin_info['username'];
            }

            // 系统guid
            $data['guid'] = get_guid_v4();
...
//        }
     ...
     * @date 2020-04-21
     */
    public function dropGuid($guid, $is_sql = false)
    {
        // 设置mark值为0
        $result = $this->where("guid",$guid)->data(['mark' => 0])->save();
        // 打印SQL
        if ($is_sql) {
            echo $this->getLastSql();
        }
...
//    }
    ...
    public function doEdit($data = [], &$error = '', $is_sql = false)
    {
        ...
        if (empty($data['update_user'])) {
            $data['update_user'] = session('userId');
        }
