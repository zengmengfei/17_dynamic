...
if (!function_exists('https_request_wx')) {
    /**
     * TODO curl提交
     * @param $url 提交链接
     * @param null $data 提交数据
     * @return array 返回数组
     */

    function https_request_wx($url, $data=null, $type = NULL)
    {
...
if (!function_exists('get_password_salt')) {
    /**
     * 获取双MD5加密密码
     ...
     * @date 2019/4/5
     */
    function get_password_salt($password, string $salt)
    {
...
if (!function_exists('get_ip_city')) {
    ...
    function get_ip_city($ip = '')
    {
        ...
        $res = @file_get_contents('http://whois.pconline.com.cn/ipJson.jsp?json=true&ip=' . $ip);
        if (empty($res)) {
            return "";
        }
        return isset($res['city']) ? $res['city'] : "";
    }
}
...
if (!function_exists('ok_msg')) {
     ...
     * @date 2020/7/1
     */
    function ok_msg($msg = "操作成功",  $data = [], $code = 0)
    {
...
if (!function_exists('err_msg')) {
     ...
     * @date 2020/7/1
     */
    function err_msg($msg = "操作失败",  $data = [], $code = 1)
    {
        $result = ['code'=>$code,'msg' => $msg, 'data' => $data,'time'=>time()];
        return json($result);
    }
 ...
 * @param string $msg 提示语
 * @param int $code 错误码
 * @return string $result json数据
 * @author PENGKING
 * @date 2020/7/1
 */
if (!function_exists('base_msg')) {
    function base_msg($msg = '',$code =1,$data=[])
    {
        $result = array(
            'code' => $code,
            'msg' => $msg,
...
if (!function_exists('upload_image')) {
    ...
    function upload_image($form_name = 'file', $save_dir = "", &$error = '')
    {
        ...
        try {
            // 允许上传的后缀
            $allowext = 'gif,GIF,jpg,JPG,jpeg,JPEG,png,PNG,bmp,BMP,ico';
            // 上传路径
...
if (!function_exists('unzip_file')) {
     ...
     * @param string $ips_string ip范围,多个用英文,分割
     * @return bool $allow 返回结果true或false
     * @author PENGKING
     * @date 2020-04-21
     */
    if (!function_exists('check_ip')) {
        function check_ip($ip = null, $ips_string = null)
        {
            $ipList = explode(',', $ips_string);
    ...
    /**
     * 获取配置表信息
     * @param int $config_id 配置表ID
     * @param string $code config_data表code
     * @return string $val 返回结果
     * @author PENGKING
     * @date 2020-04-21
     */
    if (!function_exists('get_db_config_data')) {
        function get_db_config_data($config_id = null, $code = null)
        {
            //读取缓存数据
            $cache = cache('db_config_data_' . $config_id . '_' . $code);
            $cache = [];
            if (is_empty($cache)) {
                $cache = Db::name('config_data')
                    ->where('config_id', $config_id)
                    ->where('code', $code)
                    ->value('value');
                cache('db_config_data_' . $config_id . '_' . $code, $cache, 7200);
            }
            return $cache;
        }
    ...
    if (!function_exists('get_sys_region')) {
        /**
         * 获取一级子集编码
         * @param string $code 编码
         * @return array
         */
        function get_sys_region($code = null)
        {
            if (empty($code)) {
                $code = get_db_config_data(1, "system_city");
            }
            $sys_region = cache($code . '_sys_region');
            if (empty($sys_region)) {
                $sys_region = Db::name('sys_region')->where('par_code', $code)->cache($code . '_sys_region', 3600)->select()->toArray();
            }
            return $sys_region;
        }
    }

    if (!function_exists('get_par_region_code')) {
        /**
         * 获取全部父级集编码
         * @param string $code 编码
         * @return array
         */
        function get_par_region_code($code = array(), $level = 0, $max = 4)
        {
            static $data = array();
            $data = Db::name('sys_region')->where('code', $code)->field('code,par_code,title')->select()->toArray();
            //限制max
            if (!empty($data) && $level < $max) {
                $level++;
                foreach ($data as $vv) {
                    $data = array_merge($data, get_par_region_code($vv['par_code'], $level, 4));
                }
            }
            return $data;
        }
    ...
    if (! function_exists('get_par_code')) {
        /**
         * 获取全部父级编码code数组
         * @param string $code 编码
         * @return array
         */
    function get_par_code($code =''){
           $data = cache('get_par_code_'.$code);
           if(empty($data)){
               $data =array_key_value(get_par_region_code($code),"code");
               cache('get_par_code_'.$code,$data,7200);
           }
            return $data;
        }
    }

    if (! function_exists('get_son_region_code')) {
        /**
         * 获取子集级所有编码
         * @param string $code 编码
         * @return array
         */
        function get_son_region_code($code = array(), $level = 0, $max = 4)
        {
            static $data = array();
            $data = Db::name('sys_region')->where('par_code', $code)->field('code,par_code,title')->select()->toArray();
            //限制max
            if (!empty($data) && $level < $max) {
                $level++;
                foreach ($data as $vv) {
                    $data = array_merge($data, get_son_region_code($vv['code'], $level, 4));
                }
            }
            return $data;
        }
    ...
    if (! function_exists('get_son_code')) {
        /**
         * 获取子集所有编码code数组
         * @param string $code 编码
         * @return array
         */
        function get_son_code($code =''){
            $data = cache('get_son_code_'.$code);
            if(empty($data)){
                $data =array_key_value(get_son_region_code($code),"code");
                array_push($data,$code);
                cache('get_son_code_'.$code,$data,7200);
            }
            return $data;
        }
    }
